...
$cekPoliAntrian = mysqli_query($conn, "SELECT a_antrian.keterangan FROM a_antrian WHERE a_antrian.status = '-' AND tgl_periksa = '$date' AND no_poli = 11 LIMIT 1");
...
if ($hasilCekPoli['keterangan'] == 'otomatis') {
    $ambilPoli = mysqli_query($conn, "SELECT max(a_antrian.no_urut) as nomor_antrian, a_poliklinik.nm_poli 
                    ...
                    WHERE
                    a_antrian.no_poli = 11 AND a_antrian.status = '-' AND a_antrian.keterangan = 'otomatis' AND a_antrian.tgl_periksa = '$date'");
    ...
    LEFT JOIN a_poliklinik ON a_poliklinik.id_poli = a_antrian.id_poli 
    WHERE
    a_antrian.no_poli = 11 AND a_antrian.status = '-' AND a_antrian.keterangan = 'manual' AND a_antrian.tgl_periksa = '$date' GROUP BY a_antrian.id DESC LIMIT 1");
} else {
    $ambilPoli = mysqli_query($conn, "SELECT max(a_antrian.no_urut) as nomor_antrian, a_poliklinik.nm_poli 
                    FROM a_antrian 
                    LEFT JOIN a_poliklinik ON a_poliklinik.id_poli = a_antrian.id_poli 
                    WHERE
                    a_antrian.no_poli = 11 AND a_antrian.status = '-' AND a_antrian.tgl_periksa = '$date'");
}

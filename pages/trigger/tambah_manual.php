...
$cekRuangOtomatis = mysqli_query($conn, "SELECT * FROM a_antrian WHERE tgl_periksa = '$date' AND status = '-' AND id_poli = '$id' AND keterangan = 'otomatis'");

//Cek Poli Sudah Dipakai di Antrian Manual
$cekPoliOtomatis = mysqli_query($conn, "SELECT * FROM a_antrian WHERE tgl_periksa = '$date' AND status = '-' AND no_poli = '$inputPoli' AND keterangan = 'otomatis'");
...
$hasilRuangOtomatis = mysqli_num_rows($cekRuangOtomatis);

//Hasil Cek Poli Suah Dipakai Manual
$hasilPoliOtomatis = mysqli_num_rows($cekPoliOtomatis);
...
if ($hasilAntrian >= 1) {
    ...
    $tambahAntrian = $inputNomor;
    $query = mysqli_query($conn, "INSERT INTO a_antrian (no_urut, tgl_periksa, id_poli, keterangan, status, no_poli) VALUES ('$tambahAntrian', '$date', '$id', 'manual', '-', '$inputPoli')");
    echo "$tambahAntrian";
